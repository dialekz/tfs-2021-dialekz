...
class CoffeeOrderQueryRepositoryTest extends CoffeeShopDatabaseSuite with Matchers {
  ...
  test("listTopNCustomers should return top N customers") {
    ...
    for {
      top2 <- listTopNCustomers(1)
    } yield top2 should contain theSameElementsInOrderAs Seq((1, 150.00))
  }
  ...
  test("applyDiscount should apply discount") {
    for {
      ...
      newPrice <- findOrder(1).map(_.map(_.price))
    } yield {
      assert(applied)
      assert(newPrice.contains(90))
    }
  }
  ...
  test("applyDiscount should not apply discount if order with this id does not exist") {
    for {
      applied <- applyDiscount(-1, Map("Latte" -> 0.10, "Cappuccino" -> 0.15, "Espresso" -> 0.20))
    } yield {
      assert(!applied)
  ...
  test("applyDiscount should not apply discount if discount does not exist") {
    for {
      applied <- applyDiscount(1, Map("Cappuccino" -> 0.15, "Espresso" -> 0.20))
      newPrice <- findOrder(1).map(_.map(_.price))
    } yield {
      assert(!applied)
      assert(newPrice.contains(100))
    }
  }
}
