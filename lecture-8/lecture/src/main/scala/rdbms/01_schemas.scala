...
class UsersTable(tag: Tag) extends Table[User](tag, "USERS") {
  ...
  def surname: Rep[String] = column("sname")
...
object CoffeeShopQueryRepository {
  ...
  def addCoffee(coffee: Coffee): DBIOAction[Int, NoStream, Effect.Write] =
    AllCoffees += coffee

  val expensiveCoffee: DBIOAction[Seq[Coffee], NoStream, Effect.Read] = {
    val query = AllCoffees
      .filter(table => table.price <= BigDecimal(50))
    query.result
  }
  ...
  val subquery = {
    val first10users = AllUsers.take(10).map(_.id)

    AllCoffees
      .filter(coffee => coffee.userId.in(first10users))
  }
